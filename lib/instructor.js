...
var gplay = {
  ...
  tStat:{},
  camRad:100, camThet:0, camPhi:Math.PI/6, spincam:0,
  keyY:0, keyX:0, keyZ:0, 
  paused:1, explode:0, 
  gravity:1, gravityo:0,
  vtimef:0.2,
  velfz:0.15,
  pradius:0.04,
  slow:1,
...
keysys.whenup("n"     , function(){ gplay.gravity=1 } )

keysys.whenst ("p"    , function(){ gplay.paused = (gplay.paused)^1 } )
...
keysys.whenst ("n"    , function(){ gplay.gravity=0.01 } )
keysys.whenst ("w"    , function(){ gplay.spincam = (gplay.spincam)^1 } )
keysys.whenst ("g" 
, function(){ 
  var c=gplay.gravity
  gplay.gravity=gplay.gravityo 
  gplay.gravityo=c
  } 
)
keysys.whenst ("s"    , togglestats )
keysys.whenst ("r"    , function() { writestate(); display();} )
...
function ctrlcam()
{
  ...
  var goal = 10
  if(!gplay.paused) 
    gplay.camRad=(gplay.camRad-goal)*0.996 + goal
...
function action() {
  ...
  if ((!gplay.paused)&&((gplay.scount++)%gplay.slow==0)){ 
    ...
    if (gplay.gravity){ 
      ...
      Talter.gravitybody(gplay.vtimef)
