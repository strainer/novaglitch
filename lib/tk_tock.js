/**
* @author rccomp / http://rechargecomputing.co.uk/
*/


var Tock = (function (tt) {
...
//--wrkspace
function subrostlo3(Isec,dvs,dvm,low) //checked
{ var xsec,ysec,zsec

	xsec=Isec%dvs[0]
	ysec=((Isec/dvs[0])|0)%dvs[1]
	zsec=(Isec/(dvs[0]*dvs[1]))|0
  
  //return [low[0]+xsec*dvm[0],low[1]+ysec*dvm[1],low[2]+zsec*dvm[2]]
  
  return [low[0]+xsec*dvm[0],low[1]+ysec*dvm[1],low[2]+zsec*dvm[2]]
	///this value must be the threashold of up calculation:
}
...
{ 
	...
	mksubRost( 0, 0, ((randf()*(maxgdiv-70)/2)|0)+69 ) 
	
	for( var si=0; si<Ros[1].scts; si++ )
	{ 
		var tnm=Ros[1].sctr[si+1] - Ros[1].sctr[si]
		
		if(tnm>mxgrpsz)
		{ dosubRost( 1, si, tnm ) }
		else
		{ doGroup( Ros[1].sctr[si], Ros[1].sctr[si+1] )	}
	}
}
...
function dosubRost( blv, bsc, btnm )
{ 
	var mxdivs= (btnm/(mxgrpsz*grpszfac)|0)+1
	if (mxdivs>maxgdiv-1) { mxdivs=maxgdiv-1; }
	mksubRost( blv, bsc, mxdivs ) 
	var lv=blv+1
	
	for(var si=0; si<Ros[lv].scts; si++)
	{
		var tq=Ros[lv].sctr[si+1] - Ros[lv].sctr[si] //tack quantity
		...
		{ 
			if( ((si+1)%Ros[lv].divs[0]!=0) && //next is not in new xline
					( Ros[lv].sctr[si+2] - Ros[lv].sctr[si] < mxgrpsz ) 
				) 
			{ doGroup( Ros[lv].sctr[si], Ros[lv].sctr[si+2] )
				si++ }
			else
			{ doGroup( Ros[lv].sctr[si], Ros[lv].sctr[si+1] ) }
		}
	}
}
...
{ 
	...
	Ros[0].sctr[0]=0                //first tack i
	Ros[0].sctr[1]=tkLc.length/3    //after tack i
	
	var lowx=tkLc[0], lowy=tkLc[1], lowz=tkLc[2]
	var higx=tkLc[0], higy=tkLc[1], higz=tkLc[2]  
	var I=-1
	
	for(var i=0; i<Ros[0].sctr[1]; i++)
	{ 
		Rst[i]=i
	...
	Ros[0].lobnd[0]=lowx ; Ros[0].lobnd[1]=lowy	; Ros[0].lobnd[2]=lowz	
	Ros[0].divs[0]=1     ; Ros[0].divs[1]=1     ; Ros[0].divs[2]=1
	Ros[0].divm[0]=(higx-lowx)*1.01 
	Ros[0].divm[1]=(higy-lowy)*1.01	
	Ros[0].divm[2]=(higz-lowz)*1.01 


  //dither to blur repeat-grid alignment
	var p,q
	
	p=randf()*Ros[0].divm[0]/4
	q=randf()*Ros[0].divm[0]/4
	Ros[0].lobnd[0]-=Math.abs(p)
	Ros[0].divm[0]+=Math.abs(q)+Math.abs(p)
	
	p=randf()*Ros[0].divm[1]/4
	q=randf()*Ros[0].divm[1]/4
	Ros[0].lobnd[1]-=Math.abs(p)
	Ros[0].divm[1]+=Math.abs(q)+Math.abs(p)
	
	p=randf()*Ros[0].divm[2]/4
	q=randf()*Ros[0].divm[2]/4
	Ros[0].lobnd[2]-=Math.abs(p)
	Ros[0].divm[2]+=Math.abs(q)+Math.abs(p)
...
function mksubRost( blv, bsc, scts ) //lvl uprost index , scts of divs
{ 
	var st = Ros[blv].sctr[bsc]   //starting in rostertack list
	var ov = Ros[blv].sctr[bsc+1] //finished in rostertack list
	var lv=blv+1
	
	if(Ros[lv]==null){ Ros[lv]=Rsdo() }
		
	var lw3 =subrostlo3(bsc, Ros[blv].divs, Ros[blv].divm, Ros[blv].lobnd)

	calcBestGrid(Ros[blv].divm, scts)	
	///gdivm, gdivs updated - do not alter 
  
  var bust=false
  var hi3=[  /// /// ///
	  lw3[0]+Ros[blv].divm[0],
	  lw3[1]+Ros[blv].divm[1],
	  lw3[2]+Ros[blv].divm[2]
		]  /// /// ///
	/// tweak and test
	for(i=0;i<3;i++){
		if (lw3[i]+gdivs[i]*gdivm[i]<hi3[i])
		{ gdivm[i]=floatop.inc(gdivm[i])
      //console.log("neeked")		
			if ((lw3[i]+gdivs[i]*gdivm[i]<hi3[i])
				||(lw3[i]+gdivm[i]>hi3[i]))
			{ bust=true;
  ...
  Ros[lv].divm[0]=gdivm[0]*1.0001 
  Ros[lv].divm[1]=gdivm[1]*1.0001 
  Ros[lv].divm[2]=gdivm[2]*1.0001

	Ros[lv].lobnd[0]=lw3[0]
	Ros[lv].lobnd[1]=lw3[1]
	Ros[lv].lobnd[2]=lw3[2]

  Ros[lv].divs[0]=gdivs[0]
  Ros[lv].divs[1]=gdivs[1]
  Ros[lv].divs[2]=gdivs[2]
	
	Ros[lv].scts=gdivs[0]*gdivs[1]*gdivs[2]
	scts=Ros[lv].scts
	...
	{ tka=Rst[uri]*3
		if (bust)
		{ sec=(((uri-st)*scts)/(sn+1))|0    }
		else
		{ sec=locToSector(
					 tkLc[tka], tkLc[tka+1], tkLc[tka+2],
					 Ros[lv].lobnd, Ros[lv].divm, Ros[lv].divs ) // mod scts to secure**
	  }
    
		//sec=(sec+schack)%scts	--this was really stoop	
		itkdix[uri]=Rst[uri]  //cache the tackid of the tack at uri
		itksec[uri]=sec       //the dwnsector of the tack in the roster at uri
		sctsize[sec]++

    //var lw3x =subrostlo3(sec, Ros[lv].divs, Ros[lv].divm, Ros[lv].lobnd)
		
		if((!(sec>-1&&sec<scts))
		   //~ ||tkLc[tka]<lw3x[0] || (tkLc[tka]>lw3x[0]+Ros[lv].divm)
		   //~ ||tkLc[tka+1]<lw3x[1] || (tkLc[tka+1]>lw3x[1]+Ros[lv].divm)
		   //~ ||tkLc[tka+2]<lw3x[2] || (tkLc[tka+2]>lw3x[2]+Ros[lv].divm)
			)
		//~ if(0)
		{ bd++
		  			
			if(!(sec>-1&&sec<scts))
			{ console.log("presec") }else{ console.log("posec") }
			console.log( 
			  "g=%s b=%s sc=%s bsc=%s bl=%s ri=%s ti=%s "+
				"\nxll=%s xhh=%s xdd=%s xnn=%s"+
				"\nxl=%s xh=%s xd=%s xn=%s"+
				"\nx=%s" + 
				"\nyll=%s yhh=%s ydd=%s ynn=%s"+
				"\nyl=%s yh=%s yd=%s  yn=%s"+
				"\ny=%s" +
				"\nzll=%s zhh=%s zdd=%s znn=%s"+
				"\nzl=%s zh=%s zd=%s  zn=%s"+
				"\nz=%s ",
				gd,bd,sec,bsc,blv,uri,Rst[uri], 
			  Ros[blv].lobnd[0].toFixed(3), 
				(Ros[blv].lobnd[0]+Ros[blv].divm[0]*Ros[blv].divs[0]).toFixed(3), 
				Ros[blv].divm[0].toFixed(3),Ros[blv].divs[0], 
				lw3[0].toFixed(3),hi3[0].toFixed(3), Ros[lv].divm[0].toFixed(3),Ros[lv].divs[0], 
				tkLc[tka].toFixed(3),
				Ros[blv].lobnd[1].toFixed(3), 
				(Ros[blv].lobnd[1]+Ros[blv].divm[1]*Ros[blv].divs[1]).toFixed(3),
				Ros[blv].divm[1].toFixed(3),Ros[blv].divs[1], 
				lw3[1].toFixed(3),hi3[1].toFixed(3), Ros[lv].divm[1].toFixed(3), Ros[lv].divs[1], 
				tkLc[tka+1].toFixed(3),
				Ros[blv].lobnd[2].toFixed(3), 
				(Ros[blv].lobnd[2]+Ros[blv].divm[2]*Ros[blv].divs[2]).toFixed(3),
				Ros[blv].divm[2].toFixed(3),Ros[blv].divs[2],
				lw3[2].toFixed(3),hi3[2].toFixed(3),Ros[lv].divm[2].toFixed(3),Ros[lv].divs[2],
				tkLc[tka+2].toFixed(3) 
			)	
		  debugger
		}
	  else{ gd++ }
	...
	var sst=st  //upsector start in Rst
	for(var sec=0; sec<=scts; sec++)  //loop through sector to make roster index
	{ Ros[lv].sctr[sec]=sst; sst+=sctsize[sec]; }
	
	for(var uri=st;uri<ov;uri++)  //loop through tack refs for put into roster
	{ sec=itksec[uri]  
		Rst[ Ros[lv].sctr[sec]+(sctfill[sec]++) ]
		  =itkdix[uri] 
	}
...
var qRnd=0xcb8c29e3,pRnd=0xc371d230,uni=2.328306e-10
//~ function randf() ///shr3 32 ^ cong ,8x slower than native Math.random
//~ { qRnd ^= (qRnd << 17); qRnd ^= (qRnd >>> 13); qRnd ^= (qRnd << 5); //%70
	//~ pRnd = (69069*pRnd)+1327217885                                    //%10
	//~ return (qRnd^pRnd)*uni+0.5                                        //%20
//~ }
...
tt.defoTacks      = defoTacks
tt.tksToThree     = tksToThree

tt.chopWorld      = chopWorld
tt.setGroupWorks  = setGroupWorks

tt.pulsevel       = pulsevel
tt.pulsepos       = pulsepos

tt.rndFill        = rndFill     
tt.rndRectaFill		=	rndRectaFill

tt.getState       = function(){ return { tkLc:tkLc,tkVl:tkVl,tkCl:tkCl } }
...
//~ var Tock = (function (tt) {
  ...
  //~ tt.anotherMethod=anotherMethod
	//~ return tt;
//~ }(tt));
...
outtake ={
	...
	function (f,cl,vl)
	{
		for(var i=0;i<vl.length;)
		{ 
			cl[i]=0.15+0.85/(Math.pow(Math.max(1,Math.abs(vl[i++])),f) )
			cl[i]=0.15+0.85/(Math.pow(Math.max(1,Math.abs(vl[i++])),f) )
			cl[i]=0.15+0.85/(Math.pow(Math.max(1,Math.abs(vl[i++])),f) )
		}
	},
